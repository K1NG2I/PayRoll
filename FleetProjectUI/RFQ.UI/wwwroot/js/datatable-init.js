...
$(document).ready(function () {
    $.fn.DataTable.ext.pager.numbers_length = 3;
    const table = $('#tabletest').DataTable({
        responsive: true,
        dom: 'Bfrtip',
        buttons: [
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    $('#tabletest thead th').each(function () {
                        headers.push($(this).text().trim());
                    });
                    csvData.push(headers); // push header row as array
    ...
    // Move export buttons
    table.buttons().container().appendTo('#exporttabletest');
    // Search
    $('#customSearch').on('keyup', function () {
        table.search(this.value).draw();
    });
    // Move pagination to custom div
    $('#tabletest_paginate').appendTo('#customPagination');
    // Filter dropdown logic
    $('.filter-option-applicableroute').on('click', function () {
        const value = $(this).data('value');
        const label = $(this).text();
