    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (gridTableName == 'tableuser') {
                try {
                    var useridStr = decodeURIComponent(getCookieValue('userid'));
                    var useridNum = Number(useridStr);

                    if (!Array.isArray(response?.data)) {
                        response.data = [];
                    } else {
                        const filteredUsers = response.data.filter(user => {
                            // support different casing for user id coming from API
                            const candidateId = user.userId ?? user.UserId ?? user.userid ?? user.UserID ?? null;
                            // normalize to number
                            let candidateNum = null;
                            if (candidateId === null || candidateId === undefined) {
                                candidateNum = NaN;
                            } else if (typeof candidateId === 'string') {
                                candidateNum = Number(candidateId.trim());
                            } else {
                                candidateNum = Number(candidateId);
                            }
                            // include row only when both are finite numbers and equal
                            if (Number.isFinite(candidateNum) && Number.isFinite(useridNum)) {
                                return candidateNum !== useridNum;
                            }
                            // if we can't parse id properly, keep the row
                            return true;
                        });
                        // Debug if filtering removed all items unexpectedly
                        if (filteredUsers.length === 0 && Array.isArray(response.data) && response.data.length > 0) {
                            console.debug('[FetchDataForTable] user filtering removed all users. cookie userid=', useridStr, 'candidatenum =', candidateNum, 'parsed=', useridNum, 'originalData=', response.data);
                        }
                        response.data = filteredUsers;
                    }
                } catch (ex) {
                    console.error('[FetchDataForTable] error while filtering users', ex);
                }
              
            }
            if (gridTableName == 'tabletest') {
                try {
                    var useridStr = decodeURIComponent(getCookieValue('userid'));
                    var useridNum = Number(useridStr);
                    ...
                    if (!Array.isArray(response?.data)) {
                        ...
                        const filteredUsers = response.data.filter(user => {
                            // support different casing for user id coming from API
                            const candidateId = user.testId ?? user.TestId ?? user.testid ?? user.TestID ?? null;
                            // normalize to number
                            let candidateNum = null;
                            if (candidateId === null || candidateId === undefined) {
                                candidateNum = NaN;
...
function GetGridHtml(response, gridTableName, IsEdit, IsView, IsCancel) {
    ...
    if (gridTableName == "tabletest") {
        ...
        <tr>
            ...
            if (IsEdit) {
                rowsHtml += `<a class="icon-btn" onclick="EditTest(${item.testId})"><i class="ri-edit-2-line"></i></a>`;
            }
            //if (IsView) {
            //    rowsHtml += `<a class="icon-btn" onclick="ViewUser(${item.userId})"><i class="ri-eye-line"></i></a>`;
            //}
            if (IsCancel) {
                rowsHtml += `<a class="icon-btn" onclick="DeleteTest(${item.testId})"><i class="ri-delete-bin-3-line"></i></a>`;
            }
            rowsHtml += `</td></tr>`;
        });
    }
